...
def test_sdp_full():
    Sigma = np.diag([3, 13])
    s = sdp_full(Sigma, max_iterations=1000, mu=0.99)
    ...
    s_coo = sdp_full(Sigma, max_iterations=150000, mu=0.9999)
...
def test_low_rank():
    ...
    s_cvx = cvx_sdp_full(Sigma, solver=cp.SCS, eps=1e-10)
    s_coo = sdp_low_rank(
        d, U, singular_values=None, max_iterations=10000, mu=0.999, eps=0
    )
    ...
    s_cvx = cvx_sdp_full(Sigma, solver=cp.SCS, eps=1e-10)
    s_coo = sdp_low_rank(
        d, U, singular_values=singular_values, max_iterations=2000, mu=0.995, eps=0
    )
